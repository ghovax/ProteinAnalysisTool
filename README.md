...
- `protein:set_representation_mode(mode)`: Sets visual mode (`spheres`, `backbone_trace`, `backbone_and_spheres`, `sticks`, `ball_and_stick`, `space_filling`, `lines`).
- `protein:set_color_scheme_by_property(scheme)`: Sets coloring scheme (`chain_identifier`, `chemical_element`, `bfactor_value`, `secondary_structure`).
