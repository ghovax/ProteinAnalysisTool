...
pub fn calculate_rmsd_between_coordinate_sets(
    ...
    if reference_coordinates.len() != moving_coordinates.len() {
        return Err("Coordinate sets must have the same length for RMSD calculation".to_string());
    }
...
pub fn compute_kabsch_optimal_rotation(
    ...
    if reference_centered_coordinates.len() != moving_centered_coordinates.len() {
        return Err("Coordinate sets must have the same length for Kabsch algorithm".to_string());
    }
    ...
    let mut reflection_matrix = Matrix3::identity();
